...
class GnnHybridDataset(H5Dataset):
    def __init__(self, h5file, geometry_file, k_neighbors, transforms=None, variables=None):
        """
        Args:
        ...
        """
        super().__init__(h5file, variables)
...
#        scale = np.array([100., 100., 100., 1., 1., 1., 1., 1000.])
        ...
        if self.variables is not None:
            vars = torch.tensor(self.variable_data[item], dtype=torch.float32)

        return {"data": (PyGData.Data(x=x, y=y, edge_index=knn_graph_edge_index), vars), "labels": y}
